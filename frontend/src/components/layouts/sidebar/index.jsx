...
const Sidebar = () => {
  // Media query to check if the screen width is between 769px and 900px
  let isTabletMid = useMediaQuery({
  ...
  // Effect to handle sidebar open/close based on screen size
  useEffect(() => {
    if (isMobile || isTabletMid) {
      setOpen(false); // Collapse sidebar on mobile and tablet mid-size
    } else {
      setOpen(true); // Keep sidebar open on larger screens
    }
  }, [isMobile, isTabletMid]);

  // Effect to close the sidebar when navigating to a different route (only for mobile)
  useEffect(() => {
    if (isMobile) {
      setOpen(false); // Close sidebar on route change for mobile
    }
  }, [pathname]);
  ...
  return (
    <div>
      ...
      <motion.div
        ...
        className="bg-[#333333] text-gray shadow-xl z-[999] max-w-[20rem] w-[20rem] overflow-hidden md:relative fixed h-screen"
      >
